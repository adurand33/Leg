...
.wcvr-hero h1 .robot {
  font-size: 1.2em; /* Increases ONLY the robot size (adjust between 1.6–2.2em as needed) */
  line-height: 1;
...
def fmt1(x: Optional[float], decimals: int = 1) -> str:
  if x is None:
    return "—"
  format_str = f"{x:,.{decimals}f}".replace(",", " ")
  return format_str
...
def compute_curves(cheville_cm, mollet_cm, genou_cm, cuisse_cm, taille_cm, nb_points=32):
  ...
  # heights (mm)
  h_sol      = 0.000 * v_taille # ground Level
  h_cheville = 0.039 * v_taille # joint center
  h_mollet   = 0.200 * v_taille # lower leg max
  h_genou    = 0.285 * v_taille # joint center
  h_cuisse   = 0.400 * v_taille # upper leg max
  h_fourche  = 0.500 * v_taille # leg top
  
  # Calculated Segment Lengths
  leg_height      = h_fourche - h_sol  # = 0.500 * v_taille
  tibia_length    = h_genou - h_cheville # = 0.246 * v_taille
  femur_length    = h_fourche - h_genou  # = 0.215 * v_taille
  femur_length_real   = 0.245 * v_taille   # from hip joint to knee
  ...
  # Return both curves and segment lengths
  return {
    "curves": curves,
...
if run_btn:
  if not api_key or not api_secret:
    ...
    try:
      ...
      data1 = (resp or {}).get("data", [{}])[0]

      infos     = data1.get("infos", {})
...
with col_left:
  ...
  if data1 is None:
    ...
    r0.write(f"**Jambe (cm)** : {fmt1(leg_height_cm)}")
...
with col_right:
  ...
  if data1 is None:
    ...
    if mode == "Rempli":
      draw_mesh_2d_filled(data1)
    elif mode == "Filaire" and do_unroll:
      draw_mesh_2d_wire(data1)
    elif mode == "Ombré" and not do_unroll:
      draw_mesh_3d_shaded(data1, edge_mode=edge_mode)
    elif mode == "Filaire" and not do_unroll:
      draw_mesh_3d_wire(data1)
